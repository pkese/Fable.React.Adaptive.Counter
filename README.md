...
Why would I care?
- You can now build true reactive apps without any Elmish boilerplate: no commands, dispatchers, reducers, etc., just adaptive state.
- Having structures like `List`, `Map`, `Set` yields a whole plethora of novel approaches:  
Imagine when a single element is added to existing changeable List, an adaptive descendant datastructure does not necessarily need to fully recalculate its whole state but rather just incrementally handle addition of that one extra element - and that is what FSharp.Data.Adaptive is all about.
- Ideally we would propagate *'adding a new element to a changeable list'* incrementally all the way down to *'adding a node in browser's DOM'* (optimally without even any Virtual DOM) - but that's still in design phase (React & DOM experts are kindly invited to participate in the search for optimal solutions - join the discussion at https://discordapp.com/channels/611129394764840960/624645480219148299).
